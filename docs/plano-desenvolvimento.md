...
- **Critérios de aceite:** script roda localmente sem erro sobre as 3 versões atuais; campos calculáveis batem com a contagem real dos arquivos.
- **Depende de:** 1.2

### 1.4 Gerar `meta.json` para as versões existentes ([#6](https://github.com/Projeto-Euaggelion/biblia.publica/issues/6))
- **Tamanho:** P
- **Descrição:** Rodar `gerar_meta.py` e preencher manualmente os campos não-automáticos (base textual, idioma/dialeto, anomalias conhecidas) para `blivre`, `nt-a1819a`, `nva`.
- **Critérios de aceite:** as 3 pastas têm `meta.json` válido conforme o schema de 1.2.
- **Depende de:** 1.3
...
- **Critérios de aceite:** script roda sobre as 3 versões existentes e retorna erro/exit code não-zero se encontrar problema; relatório legível no terminal.
- **Depende de:** —
...
- **Critérios de aceite:** rodando sobre as 3 versões, reporta divergência zero (ou lista exatamente as divergências conhecidas).
- **Depende de:** —
...
- **Critérios de aceite:** `.sqlite` gerado para as 3 versões, schema documentado, uma query de exemplo (buscar um versículo) funciona.
- **Depende de:** —
...
- **Critérios de aceite:** processo documentado; `CHANGELOG.md` inicial criado para as 3 versões existentes.
- **Depende de:** —
